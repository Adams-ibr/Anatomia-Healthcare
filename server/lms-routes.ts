...
// Get course by slug (public/member aware)
publicRouter.get("/courses/:slug", async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ...course, modules: modulesWithLessons, isEnrolled });
  } catch (error) {
...
superAdminRouter.delete("/users/:id", async (req: Request, res: Response) => {
  try {
    ...
    // Log the action
    const admin = await lmsStorage.getAdminUserById(currentUserId);
    const userToDelete = await lmsStorage.getAdminUserById(req.params.id);
    const success = await lmsStorage.deleteAdminUser(req.params.id);
