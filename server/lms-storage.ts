...
export class LmsStorage implements ILmsStorage {
  ...
  async getMemberById(id: string): Promise<Member | undefined> {
    const { data, error } = await supabase
      .from("members")
      .select(`
        id, email, password, firstName:first_name, lastName:last_name, 
        userType:user_type, membershipTier:membership_tier, 
  ...
  async updateMemberMembership(id: string, tier: string, expiresAt?: Date | null): Promise<Omit<Member, "password"> | undefined> {
    const { data, error } = await supabase
      ...
      .eq("id", id)
      .select("id, email, firstName:first_name, lastName:last_name, userType:user_type, membershipTier:membership_tier, membershipExpiresAt:membership_expires_at, isActive:is_active, createdAt:created_at, updatedAt:updated_at")
      .single();
